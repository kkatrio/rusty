...
fn push_write_entry(sq: &mut SubmissionQueue, fd: i32, buf: &Box<[u8]>) -> std::io::Result<()> {
        ...
        while let Some(cqe) = cq.next() {
            ...
            match event_type {
                ...
                EventType::Poll => {
                    ...
                    fd_event.insert(conn_fd, EventType::Read {buf});

                }
                EventType::Read {buf} => {
                    //println!("conn_fd in Read: {}", conn_fd);
                    push_write_entry(&mut sq, conn_fd, &buf)?;
